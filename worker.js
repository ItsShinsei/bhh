...
const ok = (d) =>
  new Response(JSON.stringify(d), { status: 200, headers: CORS });
const err = (m, s = 500) =>
  new Response(JSON.stringify({ error: m }), { status: s, headers: CORS });
const created = (d) =>
    ...
    try {
      ...
      if (path === "/api/domestik") {
        if (request.method === "POST") {
          ...
          if (!province || !city || !name)
            return err("Missing required fields: province, city, name", 400);
          const res = await env.DB.prepare(
        ...
        let q =
          p.get("all") === "1"
            ? `SELECT * FROM listings_domestik WHERE 1=1`
            : `SELECT * FROM listings_domestik WHERE ${ALIVE}`;
        const args = [];
        if (p.get("province")) {
          q += ` AND province = ?`;
          args.push(p.get("province"));
        }
        if (p.get("city")) {
          q += ` AND city = ?`;
          args.push(p.get("city"));
        }
      ...
      if (path === "/api/inter") {
        if (request.method === "POST") {
          ...
          if (!country || !city || !name)
            return err("Missing required fields: country, city, name", 400);
          const res = await env.DB.prepare(
        ...
        let q =
          p.get("all") === "1"
            ? `SELECT * FROM listings_inter WHERE 1=1`
            : `SELECT * FROM listings_inter WHERE ${ALIVE}`;
        const args = [];
        if (p.get("country")) {
          q += ` AND country = ?`;
      ...
      if (path === "/api/cruise") {
        if (request.method === "POST") {
          ...
          if (!route || !name)
            return err("Missing required fields: route, name", 400);
          const res = await env.DB.prepare(
        ...
        const q =
          p.get("all") === "1"
            ? `SELECT * FROM listings_cruise ORDER BY created_at DESC`
            : `SELECT * FROM listings_cruise WHERE ${ALIVE} ORDER BY created_at DESC`;
      ...
      if (path === "/api/umroh") {
        if (request.method === "POST") {
          ...
          if (!package_type || !name)
            return err("Missing required fields: package_type, name", 400);
          const res = await env.DB.prepare(
        ...
        let q =
          p.get("all") === "1"
            ? `SELECT * FROM listings_umroh WHERE 1=1`
            : `SELECT * FROM listings_umroh WHERE ${ALIVE}`;
        const args = [];
        if (p.get("type")) {
          q += ` AND package_type = ?`;
      ...
      if (path === "/api/offers") {
        if (request.method === "POST") {
          const body = await request.json();
          const { name, description, image_url, badge, whatsapp_msg, expires } =
            body;
        ...
        const q =
          p.get("all") === "1"
            ...
            : `SELECT * FROM offers WHERE ${ALIVE} ORDER BY created_at DESC`;
        const { results } = await env.DB.prepare(q).all();
        return ok(results);
      }
      ...
      if (path === "/api/reviews") {
        if (request.method === "POST") {
          ...
          if (!name || rating === undefined)
            return err("Missing required fields: name, rating", 400);
          const res = await env.DB.prepare(
            `INSERT INTO reviews (name, photo_url, rating, text) VALUES (?, ?, ?, ?)`,
          )
            .bind(name, photo_url || "", rating, text || "")
            .run();
          return created({ id: res.meta.last_row_id, ...body });
        }
      ...
      if (path === "/api/gallery") {
        if (request.method === "POST") {
          ...
          if (!title || !image_url || !category)
            return err(
              "Missing required fields: title, image_url, category",
              400,
            );
          const res = await env.DB.prepare(
            `INSERT INTO gallery (title, image_url, category) VALUES (?, ?, ?)`,
          )
            .bind(title, image_url, category)
            .run();
          return created({ id: res.meta.last_row_id, ...body });
        }
